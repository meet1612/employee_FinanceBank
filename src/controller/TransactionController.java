		...
		else if(operation.equals("withdrawMoney"))
		{
			...
			if(accBalance>=withdrawAmt)
			{
				...
				if(status>0)
				{
					...
					c = service1.getCustomerByID(custid);
					
					 String recipient = c.getWs_email_id();
				        String subject ="Amount withdrawn successfully";
				        String content = "A/c"+accid+" Debited by"+withdrawAmt+" Total balance :"+(accBalance-withdrawAmt);
...
//			{
				...
				if(status>0)
				{
					...
					c = service1.getCustomerByID(custid);
					 ...
					 String recipient = c.getWs_email_id();
				        String subject ="Amount deposited successfully";
				        String content = "A/c"+accid+" Credited by"+depositAmt+" Total balance :"+(accBalance+depositAmt);
